...
function get_zmk-viewer() {
  ...
  local TAG=""
  local VER=""
	local DEST_DIR=$HOME/.dotfiles/bin
  ...
  if [ "${EXT}" == "zip" ]; then
    unzip -qq -o "${TMP_DIR}/${FILENAME}" -d ./
  elif [ "${EXT}" == "tar.gz" ]; then
    tar -xf "${TMP_DIR}/${FILENAME}" --directory ./
  else
  ...
  if [ -d $DEST_DIR ]; then
    echo " - Placing zmk-viewer in $DEST_DIR"
    mv "${TMP_DIR}/zmk-viewer-${VER}-${DIST}/zmk-viewer" $DEST_DIR
    chmod +x $DEST_DIR/zmk-viewer
    ...
    echo "ERROR! $DEST_DIR is not present. Install aborted."
    rm -rf "${TMP_DIR}"
    exit 1
  fi
